...
%
% Team Members: [Kareemat Adeagbo] - Algorithim Developer
...
% Data/Atheletes/athlete2_intermediate.csv
% Data/Athletes/athletes3_advanced.csv
%
% Outputs: 
...
fprintf('Athlete 1 (Beginner): %d rows, %d columns/n', size(athlete1,1), size(athlete1,2));
fprintf('Athlete 2 (Intermediate): %d rows, %d columns/n', size(athlete2,1), size(athlete2,2));
fprintf('Athlete 3 (Advanced): %d rows, %d columns/n', size(athlete3,1), size(athlete3,2));
...
%   Resting: < 100bpm (rest days) 
%   Light: 100-130 bpm
%   Moderate: 131-160 bpm
%   Vigorous: >160bpm

% Pre-allocate a cell array to hold zone lables for each athlete allZones =
% cell(3, 1); 

for a = 1:3
    data = athletes{a};
    n = height(data);
    zones = strings(n, 1); % column of zone lables

    for i = 1:n
        hr = data.PostWorkoutHR(i); 
        if data.Duration(i) == 0
            % Rest day: PostWorkoutHR == PreWorkoutHR
            zones(i) = "Resting"; 
        elseif hr <= 130
            zones(i) = "Light";
        elseif hr <= 160 
...
% Results matrixL rows = athletes, cols = zones 
...
for a = 1:3 
    fprintf(' %s: %.2f bpm\n', a, athleteNames{a}, recoveryRate(a));
end
fprintf('\n');

%% ================ Task 5: Basic Statistics ===============
% For each athlete (exculding rest days): 
%   - Mean PostWorkoutHR
%   - Mean Duration 
%   - Std of Intesnity (consistency; lower = more consistent) 
...
fprintf('1. Most time in Vigorous zone: %s (%.0f min)\n', ...
    athleteNames{maxVigIdx}, timeInZone(maxVigIdx, 4));
% Q2: Most consistent training (lowest std of intensity) 
[~, mostConsistIdx] = min(stdIntensity); 
fprintf('2. Most consistent training: %s (std = %.2f)\n', ...
...
fprintf('3. Best cardiovascular efficiency: %s (%.2f bpm avg increase)\n', ...
    athleteNames{bestCardioIdx}, recoveryRate(bestCardioIdx)); 
% Q4: Workout pattern summary 
fprintf('4. Workout patterns by fitness level:\n'); 
for a = 1:3
    fprintf(' %s: %d workouts over 21 days, avg %.1f min, avg intensity %.1f\n', ...
        athleteNames{a}, totalWorkouts(a), meanDuration(a), meanDuration(a), meanIntensity(a)); 
end 
fprintf('\n'); 
...
% Computer a single 0-100 fitness score per athlete using four measured
% metrics: 1). Total worout time 2). Average intensity 3). Consistency 4).
% Recovery efficiency 

% All four metrics are weighted equally (25% each_. 
% Helper: safe normalize (direct) ---
% Maps a vector of values to [0, 100]; higher raw values = high score
normalizeDir = @(v) deal( ...
    (max(v) - min(v)) < 1e-9, ... % flag: all values equal
    (v - min(v)) ./ max(max(v) - min(v), 1e-9) * 100);

% --- Helper: safe normalize (inverse) ---
% Maps a vector of value to [0, 100]; lower raw value - higher score 
normalizeInv = @(v) deal( ...
    (max(v) - min(v)) < 1e-9, ...
    (max(v) - v) ./ max(max(v) - min(v), 1e-9) * 100);

% --- Compute each normalize component ---
% (1) Total workout time - more minutes = fitter 
[eqFlag, scoreTime] = normalizeDir(totalMinutes);
if eqFlag, scoreTime = ones(1,3) * 50; end 

% (2) Average intensity - higher effort = fitter 
[eqFlag, scoreIntensity] = normalizeDir(meanIntensity);
if eqFlag, scoreIntensity = ones(1,3) * 50; end 

% (3) Consistency - lower std of intensity = more consistency 
[eqFlag, scoreConsistency] = normalizeInv(stdIntensity); 
if eqFlag, scoreConsistency = ones(1,3) * 50, end
% (4) Recovery efficiency - lower HR increase = better cardiovascular
[eqFlag, scoreRecovery] = normalizeInv(stdIntensity); 
if eqFlag, scoreRecovery = ones(1,3) * 50, end

% --- Compute each normalized component ---

% (1) Total workout time - more minutes = fitter 
 [eqFlag, scoreTime] = normalizeDir(totalMinutes);
 if eqFlag, scoreTime = ones(1,3) * 50, end

 %(2) Average intensity - higger effort = fitter
 [eqFlag, scoreIntensity] = normalizeDir(meanIntensity);
 if eqFlag, scoreIntensity = ones(1,3) * 50; end 

 % (3) Consistency - lower std of intensity = more consistent = fitter 
 [eqFlag, scoreConsistency] = normalizeInv(stdIntensity); 
 if eqFlag, scoreConsistency = ones(1,3) * 50, end
 ...
 % ---- Combine with equal weighting (25% each) ---
  ...
  fprintf('%-15s %10s %11s %13s %12 | %10s\n', ...
  ...
  % --- Add composite scores to su,,ary table and re-save --- 
  compositeScore = max(compositeScore', 1);
  summaryTable.compositeScore = compositeScore;
  writetable(summaryTable.csvOut); 
  fprintf(['Update summary table (with composite scores) saved to: %s' ...
      \n'], csvOut);
  % Re-save workspace with composite score variables included 
  save(matOut); 
  fprintf('Workspace re-saved t0: %s\n', matOut); 
